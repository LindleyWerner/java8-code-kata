...
public class Exercise9Test extends ClassicOnlineStore
{
    ...
    public void simplestStringJoin()
    {
        ...
        Collector<String, ?, String> toCsv = new CollectorImpl<>(supplier, accumulator, combiner, finisher, Collections.emptySet());
    ...
    public void mapKeyedByItems()
    {
        ...
        BinaryOperator<Map<String, Set<String>>> combiner = (left, right) -> {
            right.forEach(
                    (item, customers) -> left.merge(item, customers, (oldVal, newVal) -> {
        ...
        Collector<Customer, ?, Map<String, Set<String>>> toItemAsKey = new CollectorImpl<>(supplier, accumulator, combiner, finisher, EnumSet.of(
    ...
    public void bitList2BitString()
    {
        ...
        String acumulator = "";
        StringBuilder myBitString = new StringBuilder("");
        Boolean sequenceFlag = false;
        int start = 0, end = 0;
        bitList += ",";
        for (char c : bitList.toCharArray())
        {
            if (c != '-' && c != ',')
            {
                acumulator += c;
            ...
            else
            {
                ...
                {
                    sequenceFlag = true;
                    start = Integer.parseInt(acumulator);
                    acumulator = "";
                }
                else
                {
                    end = Integer.parseInt(acumulator);
                    acumulator = "";
                    if (myBitString.length() < end)
                    {
                        myBitString.setLength(end + 1);
                    }
                    if (sequenceFlag)
                    {
                        for (int i = start; i <= end; i++)
                        {
                            myBitString.setCharAt(i, '1');
                        }
                    }
                    else
                    {
                        myBitString.setCharAt(end, '1');
                    }
                    sequenceFlag = false;
                }
            }
        ...
        for (int i = 0; i < myBitString.length(); i++)
        {
            if (myBitString.charAt(i) == '\u0000')
            {
                myBitString.setCharAt(i, '0');
            }
        }

        //String bitString = Arrays.stream(bitList.split(",")).collect(toBitString);
        assertThat(myBitString.substring(1), is("01011000101001111000011100000000100001110111010101")
